...
//
//Args, 1) Product 2) Owner 4) start
// zero all other contract fields 
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	if len(args) != 1 {
		return nil, errors.New("Incorrect number of arguments. Expecting 1")
	}

	err := stub.PutState("Status", "Proposal")

	err := stub.PutState("Product", []byte(args[0]))

	err := stub.PutState("Owner", []byte(args[1]))

	err := stub.PutState("StartDate", []byte(args[2]))
	if err != nil {
		return nil, err
	}
